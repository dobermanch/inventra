...
try {
  ...
  for (const exp of expensesWithInvoices) {
    db.prepare('INSERT INTO expense_invoices (expense_id, name, url) VALUES (?, ?, ?)')
      .run(exp.id, exp.invoice_name || exp.picture_url.split('/').pop(), exp.picture_url);
  }
...
async function startServer() {
  ...
  app.get('/api/dashboard', (req, res) => {
    try {
      const totalSales = db.prepare("SELECT SUM(total_amount) as total FROM orders WHERE status NOT IN ('canceled', 'returned')").get() as any;
      const activeOrders = db.prepare("SELECT COUNT(*) as count FROM orders WHERE status = 'active'").get() as any;
      ...
      `).all();
      const recentExpenses = db.prepare('SELECT * FROM expenses ORDER BY date DESC LIMIT 5').all();
  ...
  app.post('/api/inventory/restock', (req, res) => {
    ...
    const transaction = db.transaction(() => {
      db.prepare('UPDATE item_variants SET stock_count = stock_count + ? WHERE id = ?').run(quantity, variant_id);
      db.prepare('INSERT INTO restock_history (variant_id, quantity, cost_per_unit) VALUES (?, ?, ?)').run(variant_id, quantity, cost_per_unit);
    });
  ...
  app.post('/api/orders', (req, res) => {
    ...
    const transaction = db.transaction(() => {
      ...
      for (const item of items) {
        insertOrderItem.run(orderId, item.variant_id, item.quantity, item.unit_price);
  ...
  app.put('/api/orders/:id', (req, res) => {
    try {
      ...
      const transaction = db.transaction(() => {
        ...
        const insertOrderItem = db.prepare('INSERT INTO order_items (order_id, variant_id, quantity, unit_price) VALUES (?, ?, ?, ?)');
        const deductStock = db.prepare('UPDATE item_variants SET stock_count = stock_count - ? WHERE id = ?');
        
        for (const item of items) {
          insertOrderItem.run(id, item.variant_id, item.quantity, item.unit_price);
  ...
  app.patch('/api/orders/:id/status', (req, res) => {
    ...
    const transaction = db.transaction(() => {
      ...
      db.prepare('UPDATE orders SET status = ?, returned_at = ? WHERE id = ?')
        .run(status, status === 'returned' ? new Date().toISOString() : null, id);
    });
  ...
  app.get('/api/expenses', (req, res) => {
    try {
      ...
      const invoices = db.prepare('SELECT * FROM expense_invoices').all() as any[];
  ...
  app.post('/api/expenses', upload.array('invoices'), (req, res) => {
    try {
      const { name, details, amount, quantity, total_amount, date, category, subcategory } = req.body;
      const invoiceNames = JSON.parse(req.body.invoiceNames || '[]');
      
      const info = db.prepare('INSERT INTO expenses (name, details, amount, quantity, total_amount, date, category, subcategory) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        .run(name, details, parseFloat(amount) || 0, parseFloat(quantity) || 1, parseFloat(total_amount) || 0, date, category, subcategory);
  ...
  app.put('/api/expenses/:id', upload.array('invoices'), (req, res) => {
    try {
      ...
      // Update basic info
      db.prepare('UPDATE expenses SET name = ?, details = ?, amount = ?, quantity = ?, total_amount = ?, date = ?, category = ?, subcategory = ? WHERE id = ?')
        .run(name, details, parseFloat(amount) || 0, parseFloat(quantity) || 1, parseFloat(total_amount) || 0, date, category, subcategory, id);

      // Update existing invoice names
      existingInvoices.forEach((inv: any) => {
        db.prepare('UPDATE expense_invoices SET name = ? WHERE id = ?').run(inv.name, inv.id);
      });
