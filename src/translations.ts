export const translations = {
  en: {
    dashboard: "Dashboard",
    inventory: "Inventory",
    orders: "Orders",
    sales: "Sales History",
    expenses: "Expenses",
    settings: "Settings",
    totalSales: "Total Sales",
    activeOrders: "Active Orders",
    shippedOrders: "Shipped Orders",
    lowStockItems: "Low Stock Items",
    recentExpenses: "Recent Expenses",
    dashboardOverview: "Dashboard Overview",
    salesTrends: "Sales Trends",
    lowStockAlerts: "Low Stock Alerts",
    allStocked: "All items are well stocked.",
    addExpense: "Add Expense",
    expenseName: "Expense Name",
    category: "Category",
    subcategory: "Subcategory",
    details: "Details",
    amount: "Amount",
    quantity: "Quantity",
    totalPrice: "Total Price",
    date: "Date",
    actions: "Actions",
    save: "Save",
    cancel: "Cancel",
    edit: "Edit",
    delete: "Delete",
    language: "Language",
    currency: "Currency",
    saveSettings: "Save Settings",
    english: "English",
    ukrainian: "Ukrainian",
    inventoryManagement: "Inventory Management",
    addInventory: "Add Inventory",
    orderManagement: "Order Management",
    newOrder: "New Order",
    customerName: "Customer Name",
    phone: "Phone",
    email: "Email",
    address: "Address",
    status: "Status",
    items: "Items",
    total: "Total",
    discount: "Discount",
    createOrder: "Create Order",
    saveChanges: "Save Changes",
    searchItem: "Search Item",
    stockBySize: "Stock by Size",
    price: "Price",
    lowStockThreshold: "Low Stock Threshold",
    description: "Description",
    uploadImage: "Upload Image",
    uploadInvoice: "Upload Attachment",
    downloadInvoice: "Download Attachment",
    downloadAll: "Download All",
    invoiceName: "Attachment Name",
    dragAndDrop: "Drag and drop files here or click to select",
    uploadedInvoices: "Attachments",
    nameRequired: "Name is required",
    categoryRequired: "Category is required",
    areYouSureDelete: "Are you sure you want to delete this?",
    confirmDelete: "Confirm Delete",
    groupByCategory: "Group by Category",
    filterByCategory: "Filter by Category",
    allCategories: "All Categories",
    groupBy: "Group By",
    noGrouping: "None",
    byYear: "Year",
    byMonthYear: "Month & Year",
    bySubcategory: "Subcategory",
    byCategory: "Category",
    filterByStatus: "Filter by Status",
    allStatuses: "All Statuses",
    byStatus: "Status",
    searchOrders: "Search by name, phone, email, address…",
    searchInventory: "Search by name, description…",
    searchExpenses: "Search by name, details…",
    editOrder: "Edit Order",
    markAsShipped: "Mark as Shipped",
    markAsDelivered: "Mark as Delivered",
    markAsCanceled: "Mark as Canceled",
    markAsReturned: "Mark as Returned",
    removeItem: "Remove Item",
    editItem: "Edit Item",
    deleteItem: "Delete Item",
    restockItem: "Click to restock",
    editExpense: "Edit Expense",
    deleteExpense: "Delete Expense",
    salesHistory: "Sales History",
    totalRevenue: "Total Revenue",
    orderId: "Order ID",
    orderDetails: "Order Details",
    close: "Close",
    customerDetails: "Customer Details",
    notes: "Notes",
    subtotal: "Subtotal",
    name: "Name",
    allOrders: "All Orders",
    searchSales: "Search by name, phone, email, notes…",
    statusActive: "Active",
    statusShipped: "Shipped",
    statusDelivered: "Delivered",
    statusCanceled: "Canceled",
    statusReturned: "Returned",
    currentMonthSales: "This Month Sales",
    recentActiveOrders: "Recent Active Orders",
    noActiveOrders: "No active or shipped orders",
    noRecentExpenses: "No recent expenses",
    loading: "Loading...",
    salesChartLabel: "Sales",
    noSalesData: "No sales data yet",
    naValue: "N/A",
    stockLeft: "left",
    size: "Size",
    stock: "Stock",
    editInventoryItem: "Edit Inventory Item",
    addNewInventoryItem: "Add New Inventory Item",
    itemName: "Item Name",
    itemNameRequired: "Item name is required",
    productImage: "Product Image",
    orImageUrl: "Or Image URL",
    variantsTitle: "Variants (Sizes & Initial Stock)",
    addVariant: "Add Variant",
    restockItemTitle: "Restock Item",
    addingStockForSize: "Adding stock for size:",
    quantityToAdd: "Quantity to Add",
    addStock: "Add Stock",
    editOrderPrefix: "Edit Order #",
    createNewOrder: "Create New Order",
    phoneRequired: "Phone is required",
    addressRequired: "Address is required",
    addItems: "Add Items",
    qty: "Qty",
    uncategorized: "Uncategorized",
    chooseLanguageDescription: "Choose your preferred language for the interface.",
    defaultCurrencyDescription: "Default currency for sales and expenses.",
    appMotto: "Track. Sell. Grow.",
  },
  ua: {
    dashboard: "Панель приладів",
    inventory: "Інвентар",
    orders: "Замовлення",
    sales: "Історія продажів",
    expenses: "Витрати",
    settings: "Налаштування",
    totalSales: "Загальні продажі",
    activeOrders: "Активні замовлення",
    shippedOrders: "Відправлені замовлення",
    lowStockItems: "Товари з низьким запасом",
    recentExpenses: "Останні витрати",
    dashboardOverview: "Огляд панелі приладів",
    salesTrends: "Тренди продажів",
    lowStockAlerts: "Попередження про низький запас",
    allStocked: "Всі товари в достатній кількості.",
    addExpense: "Додати витрату",
    expenseName: "Назва витрати",
    category: "Категорія",
    subcategory: "Підкатегорія",
    details: "Деталі",
    amount: "Сума",
    quantity: "Кількість",
    totalPrice: "Загальна вартість",
    date: "Дата",
    actions: "Дії",
    save: "Зберегти",
    cancel: "Скасувати",
    edit: "Редагувати",
    delete: "Видалити",
    language: "Мова",
    currency: "Валюта",
    saveSettings: "Зберегти налаштування",
    english: "Англійська",
    ukrainian: "Українська",
    inventoryManagement: "Управління інвентарем",
    addInventory: "Додати товар",
    orderManagement: "Управління замовленнями",
    newOrder: "Нове замовлення",
    customerName: "Ім'я клієнта",
    phone: "Телефон",
    email: "Email",
    address: "Адреса",
    status: "Статус",
    items: "Товари",
    total: "Разом",
    discount: "Знижка",
    createOrder: "Створити замовлення",
    saveChanges: "Зберегти зміни",
    searchItem: "Пошук товару",
    stockBySize: "Запас за розміром",
    price: "Ціна",
    lowStockThreshold: "Поріг низького запасу",
    description: "Опис",
    uploadImage: "Завантажити зображення",
    uploadInvoice: "Завантажити вкладення",
    downloadInvoice: "Завантажити вкладення",
    downloadAll: "Завантажити все",
    invoiceName: "Назва вкладення",
    dragAndDrop: "Перетягніть файли сюди або натисніть, щоб вибрати",
    uploadedInvoices: "Вкладення",
    nameRequired: "Назва обов'язкова",
    categoryRequired: "Категорія обов'язкова",
    areYouSureDelete: "Ви впевнені, що хочете видалити це?",
    confirmDelete: "Підтвердити видалення",
    groupByCategory: "Групувати за категорією",
    filterByCategory: "Фільтрувати за категорією",
    allCategories: "Всі категорії",
    groupBy: "Групувати за",
    noGrouping: "Без групування",
    byYear: "Роком",
    byMonthYear: "Місяцем та роком",
    bySubcategory: "Підкатегорією",
    byCategory: "Категорією",
    filterByStatus: "Фільтрувати за статусом",
    allStatuses: "Всі статуси",
    byStatus: "Статусом",
    searchOrders: "Пошук за ім'ям, телефоном, email, адресою…",
    searchInventory: "Пошук за назвою, описом…",
    searchExpenses: "Пошук за назвою, деталями…",
    editOrder: "Редагувати замовлення",
    markAsShipped: "Позначити як відправлено",
    markAsDelivered: "Позначити як доставлено",
    markAsCanceled: "Позначити як скасовано",
    markAsReturned: "Позначити як повернуто",
    removeItem: "Видалити позицію",
    editItem: "Редагувати товар",
    deleteItem: "Видалити товар",
    restockItem: "Натисніть для поповнення",
    editExpense: "Редагувати витрату",
    deleteExpense: "Видалити витрату",
    salesHistory: "Історія продажів",
    totalRevenue: "Загальний дохід",
    orderId: "№ замовлення",
    orderDetails: "Деталі замовлення",
    close: "Закрити",
    customerDetails: "Дані клієнта",
    notes: "Нотатки",
    subtotal: "Підсумок",
    name: "Ім'я",
    allOrders: "Всі замовлення",
    searchSales: "Пошук за ім'ям, телефоном, email, нотатками…",
    statusActive: "Активний",
    statusShipped: "Відправлено",
    statusDelivered: "Доставлено",
    statusCanceled: "Скасовано",
    statusReturned: "Повернуто",
    currentMonthSales: "Продажі цього місяця",
    recentActiveOrders: "Останні активні замовлення",
    noActiveOrders: "Немає активних або відвантажених замовлень",
    noRecentExpenses: "Немає останніх витрат",
    loading: "Завантаження...",
    salesChartLabel: "Продажі",
    noSalesData: "Даних про продажі поки немає",
    naValue: "Н/Д",
    stockLeft: "залишилось",
    size: "Розмір",
    stock: "Запас",
    editInventoryItem: "Редагувати товар",
    addNewInventoryItem: "Додати новий товар",
    itemName: "Назва товару",
    itemNameRequired: "Назва товару обов'язкова",
    productImage: "Зображення товару",
    orImageUrl: "Або URL зображення",
    variantsTitle: "Варіанти (розміри та початковий запас)",
    addVariant: "Додати варіант",
    restockItemTitle: "Поповнити запас",
    addingStockForSize: "Поповнення для розміру:",
    quantityToAdd: "Кількість для додавання",
    addStock: "Додати запас",
    editOrderPrefix: "Редагувати замовлення №",
    createNewOrder: "Створити нове замовлення",
    phoneRequired: "Телефон обов'язковий",
    addressRequired: "Адреса обов'язкова",
    addItems: "Додати товари",
    qty: "Кіл-сть",
    uncategorized: "Без категорії",
    chooseLanguageDescription: "Виберіть бажану мову інтерфейсу.",
    defaultCurrencyDescription: "Валюта за замовчуванням для продажів та витрат.",
    appMotto: "Відстежуй. Продавай. Зростай.",
  },
};

export type Language = "en" | "ua";
export type TranslationKey = keyof typeof translations.en;
